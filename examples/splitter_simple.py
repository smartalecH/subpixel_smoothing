...
dx = 2.0
dy = 2.0
filename = "splitter_no_averaging"
min_length = 0.09
...
waveguide_geometry = [mp.Block(material=Si, center=mp.Vector3(-sx/2),
                      ...
                      mp.Block(material=Si, center=mp.Vector3(sx/2,wy),
                               size=mp.Vector3(sx,w,mp.inf)),
                      mp.Block(material=Si, center=mp.Vector3(sx/2,-wy),
                               size=mp.Vector3(sx,w,mp.inf))         
                            ]
...
obj_list = [mpa.EigenmodeCoefficient(sim,
                                    mp.Volume(center=mp.Vector3(-dy/2-0.5),
                                    size=mp.Vector3(0,2*mon_size,0)),
                                    1,
                                    eig_parity=eig_parity),
            mpa.EigenmodeCoefficient(sim,
                                    mp.Volume(center=mp.Vector3(dy/2+0.5,wy),
...
def J(input,top,bottom):
    top_err = npa.abs(0.5-npa.abs(top/input)**2)**2
    bot_err = npa.abs(0.5-npa.abs(bottom/input)**2)**2
